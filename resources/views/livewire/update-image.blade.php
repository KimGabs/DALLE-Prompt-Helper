...
<div class="content-center px-5">
    <div class="flex mb-4 w-full max-w h-80 bg-gray-200 mt-3 md:mt-0">
        ...
        @else($post->image)
            <img wire:model='image' src="{{ $post->image instanceof \Livewire\TemporaryUploadedFile ? $post->image->temporaryUrl() : asset('storage/' . $post->image) }}" alt="" class="object-contain w-full h-full dark:bg-gray-800">
        @endif
    </div>
